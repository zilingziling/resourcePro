...
import { saveAttend, reportPlatform, saveKnowledge, getCourseResourceList, saveFile, deletecurriculum, deletefile, release, report } from '../../../api/personals';
...
class TeacherTableData extends Component {
    constructor(props) {
        super(props);
    ...
    previewVideo(record) {
        let previewVideotitle = record.name || '视频预览';
    ...
    async getTableData(paginat) {
        ...
        let res = await getCourseResourceList(window.sessionStorage.getItem('token') || '', window.sessionStorage.getItem('userId'), page, limit);
        if (!res) {
            return
        }
        let pagination = {
